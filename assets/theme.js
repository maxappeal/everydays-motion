...
(function(){
  ...
  theme.Variants = (function() {
    ...
    function Variants(options) {
      ...
      window.pdpTrustpilotStarsCache = false;

      this.container.querySelectorAll(this.singleOptionSelector).forEach(el => {
        el.addEventListener('change', () => {
          const qty = document.querySelector('.variant-qty.js .variant-qty__value');
          if (qty) {
            let value = parseInt(el.dataset.packagingQuantity);

            if (value > 1) {
              qty.textContent = value + ' ' + qty.dataset.plular;
            } else {
              if (qty.hasAttribute('data-exception')) {
                if (el.value === qty.dataset.exception) {
                  qty.textContent = value + ' ' + qty.dataset.exception;
                } else {
                  qty.textContent = value + ' ' + qty.dataset.singular;
                }
              } else {
                qty.textContent = value + ' ' + qty.dataset.singular;
              }
          ...
          setTimeout(() => {
            const slsOptions = document.querySelectorAll('.sealsubs-container > .sls-select > option');
            if (slsOptions.length) {
              slsOptions.forEach(option => {
                option.removeAttribute('disabled');
              });
            }
          }, 1000);

          let pdpTrustpilotStars = document.querySelector('.product-block.trustpilot__stars');
          if (pdpTrustpilotStars) {
            if (window.pdpTrustpilotStarsCache == false) {
              window.pdpTrustpilotStarsCache = pdpTrustpilotStars.innerHTML;
            }
            setTimeout(()=>{
              pdpTrustpilotStars.innerHTML = window.pdpTrustpilotStarsCache;
            }, 50);
          }
        });
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(text) {
        ...
        if (typeof window.onCartUpdate === 'function') {
          window.onCartUpdate(+subtotal, count);
        }
      },
  ...
  theme.collapsibles = (function() {
    ...
    function toggle(evt) {
      ...
      var imageEl = el.parentNode.querySelector(selectors.image);
      ...
      if (imageEl) {
        var imageId = imageEl.getAttribute('data-target');
        var imageTarget = document.getElementById(imageId);

        imageTarget.setAttribute('src', imageEl.dataset.src);
        if (imageEl.dataset.srcset) {
          imageTarget.setAttribute('srcset', imageEl.dataset.srcset);
        }
      }
  ...
  theme.Drawers = (function() {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      open: function(evt, returnFocusEl) {
        ...
        const mesContainers = document.querySelectorAll('#moin-ai-container, ._hj-widget-container');
        if(mesContainers.length) mesContainers.forEach(mesContainer => {mesContainer.style.display = 'none'});
      ...
      close: function(evt) {
        ...
        const mesContainers = document.querySelectorAll('#moin-ai-container, ._hj-widget-container');
        if(mesContainers.length) mesContainers.forEach(mesContainer => {mesContainer.style.display = 'block'});
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        piecePriceWrapper: '[data-piece-price-wrapper]',
        piecePrice: '[data-piece-price]',
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      rearrangePaymentButtons: function() {
        const paymentButtons = this.container.querySelector(selectors.paymentButtons);
        if (paymentButtons) {
          // create mutation observer to detect when more options button is added
          // then move more options button to end of payment buttons
          const observer = new MutationObserver(function(mutations) {
            console.log('mutation');
            mutations.forEach(function(mutation) {
              if (mutation.addedNodes.length) {
                console.log('added')
                const moreOptions = paymentButtons.querySelector('.shopify-payment-button__more-options');
                if (moreOptions) {
                  paymentButtons.appendChild(moreOptions);
                  // remove observer after more options button is moved
                  observer.disconnect();
                }
              }
            });
          });
          observer.observe(paymentButtons, { childList: true, subtree: true });
        }
      },
      ...
      updateUnitPrice: function(evt) {
        ...
        if (variant && variant.title.includes('(')) {
          this.container.querySelector(this.selectors.piecePriceWrapper).classList.remove(classes.hidden);
          this.container.querySelector(this.selectors.piecePrice).innerHTML = (variant.title).split('(').slice(-1).toString().split(')')[0];
        } else {
          this.container.querySelector(this.selectors.piecePrice).innerHTML = variant.title;
          this.container.querySelector(this.selectors.piecePriceWrapper).classList.add(classes.hidden);
        }
      },
      ...
      updateInventory: function(evt) {
        ...
        if (!variant || !variant.inventory_management || variant.inventory_policy === 'continue') {
          this.toggleInventoryQuantity(variant, false, evt);
        ...
        if (variant.inventory_management === 'shopify' && window.inventories && window.inventories[this.productId]) {
          ...
          this.toggleInventoryQuantity(variant, variantInventoryObject, evt);
      ...
      toggleInventoryQuantity: function(variant, variantInventoryObject, event) {
        const { quantity, policy } = variantInventoryObject || {};
        if (!this.settings.inventory) {
          return;
        }

        const hasV2Status = event.target.querySelector('.product-block--sales-point--v2');
